...
export class AppointmentService {
    ...
    rescheduleAppointment(professionalId: number, patientName: string, currentDate: Date, newDate: Date) {
        const currentAppointment = this.getAppointmentsForProfessional(professionalId, currentDate, patientName);
        if (!currentAppointment) {
            throw new Error('Agendamento não encontrado para remarcação');
        }

        const sameDate = currentDate.getTime() === newDate.getTime();
        if (!sameDate && !this.checkAvailability(professionalId, newDate)) {
            throw new Error('Novo horário indisponível para este profissional');
        }

        currentAppointment.date = newDate.toISOString();
