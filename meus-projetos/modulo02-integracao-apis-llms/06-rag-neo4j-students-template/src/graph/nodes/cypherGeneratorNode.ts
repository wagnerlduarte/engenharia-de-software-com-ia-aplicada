  ...
  return async (state: GraphState): Promise<Partial<GraphState>> => {
    try {
      ...
      const { data, error } = await callStructuredWithTimeout(
        ...
        config.llmTimeoutMs,
        "Cypher generation timed out",
      );

      if (error) {
        return {
          error: `Failed to generate Cypher query: ${error ?? "Unknown error"}`,
        };
      }
      ...
      if (typeof rawQuery !== "string") {
        return {
          error: "Failed to generate Cypher query: model returned invalid query type",
        };
      }

      const generatedQuery = rawQuery.trim();

      if (!generatedQuery) {
        return {
          error: "Failed to generate Cypher query: model returned empty query",
        };
      }
