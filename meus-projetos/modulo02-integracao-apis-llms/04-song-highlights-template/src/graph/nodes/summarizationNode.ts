...
export function createSummarizationNode(
  ...
  return async (
    ...
    const previewsSummary = state.conversationSummary as
      | ConversationSummary
      | undefined;
    ...
    if (result.error || !result.data) {
      console.error("LLM summarization failed", result.error);
      return {
        needsSummarization: false, // avoid retrying summarization immediately
      };
    }
